    ...
    <div class="modal-dialog modal-dialog-centered text-center modal-lg" role="document">
        <div class="modal-content modal-content-demo expanel expanel-primary">
            <div class="modal-header expanel-heading">
                <h6 class="modal-title">Edit Gallery</h6><button aria-label="Close" class="btn-close text-white" data-bs-dismiss="modal" ><span aria-hidden="true">&times;</span></button>
            </div>
                ...
                <div class="modal-body expanel-body text-start">
                    <div class="row">
                        <div class="col form-group">
                            ...
                            <input class="form-control" type="hidden" value="'.$gallery['gal_id'].'"  name="gal_id" required>
                            <input class="form-control" type="text" value="'.$gallery['gal_title'].'"  name="gal_title" required>
                        </div>
                        <div class="col form-group">
                            <label class="form-label" for="card-name">Ordering <span class="text-danger">*</span></label>
                            <input class="form-control" name="gal_ordering" value="'.$gallery['gal_ordering'].'" readonly type="number" required>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col form-group">
                            <label class="form-label" for="card-name">Date <span class="text-danger">*</span></label>
                            <input class="form-control" name="gal_dated" value="'.$gallery['gal_dated'].'" type="date" required>
                        </div>
                        <div class="col form-group">
                            ...
                            <select class="form-control select2-show-search form-select" required name="id_cat" data-placeholder="Select">
                                ...
                                foreach($categories as $cat):
                                    echo '
                                    <option value="'.$cat['cat_id'].'" '.($cat['cat_id'] == $gallery['id_cat'] ? 'selected' : '').'>'.$cat['cat_name'].'</option>';
                                endforeach;
                                echo '
                            </select>
                        </div>
                        ...
                        <div class="col form-group">
                            ...
                            <select class="form-control select2-show-search form-select" required name="id_place"  data-placeholder="Select">
                                ...
                                foreach($places as $place):
                                    echo '
                                    <option value="'.$place['place_id'].'" '.($place['place_id'] == $gallery['id_place'] ? 'selected' : '').'>'.$place['place_address'].'</option>';
                                endforeach;
                                echo '
                            </select>
                        </div>
                        <div class="col form-group">
                            ...
                            <select class="form-control select2-show-search form-select" required name="id_file_type" onchange="changeFileType(this.value)" data-placeholder="Select">
                                ...
                                foreach($fileTypes as $key => $type):
                                    echo '
                                    <option value="'.$key.'" '.($key == $gallery['id_file_type'] ? 'selected' : '').'>'.$type.'</option>';
                                endforeach;
                                echo '
                            </select>
                        </div>
                    ...
                    <div class="row">
                        <div class="col form-group" id="image" '.($gallery['id_file_type'] == '2' ? 'style="display:none;"' : '').'>
                            <label class="form-label">Image </label>
                            <input type="file" name="gal_image_video" id="image_field" accept="image/*" class="form-control" />
                        </div>
                        <div class="col form-group" id="video" '.($gallery['id_file_type'] == '1' ? 'style="display:none;"' : '').'>
                            <label class="form-label">Youtube Code <span class="text-danger">*</span></label>
                            <input type="text" name="gal_image_video" value="'.$gallery['gal_image_video'].'" id="video_field" class="form-control" />
                        </div>
                        <div class="col form-group">
                            ...
                            <select class="form-control select2-show-search form-select" required name="gal_status" data-placeholder="Select">
                                ...
                                foreach($statuses as $key => $status):
                                    echo '
                                    <option value="'.$key.'" '.($key == $gallery['gal_status'] ? 'selected' : '').'>'.$status.'</option>';
                                endforeach;
                                echo '
                            </select>
                        </div>
                    ...
                    <div class="row">
                        <div class="col form-group">
                            ...
                            <textarea name="gal_description" rows="5" class="form-control">'.$gallery['gal_description'].'</textarea>
                        </div>
                    </div>
                </div>
