<?php 
    ...
    $condition = array ( 
                            ...
                            'where'         =>  array(
                                                        'testimonials_id'  =>  $_GET['testimonials_id']
                                                    ), 
                            'return_type'   =>  'single'
                       ); 
    ...
    <div class="modal-dialog modal-dialog-centered text-center modal-lg" role="document">
        <div class="modal-content modal-content-demo expanel expanel-primary">
            <div class="modal-header expanel-heading">
                <h6 class="modal-title">Edit Testimonials </h6><button aria-label="Close" class="btn-close text-white" data-bs-dismiss="modal" ><span aria-hidden="true">&times;</span></button>
            </div>
                ...
                <div class="modal-body expanel-body text-start">
                    <div class="row">
                        <div class="col form-group">
                            ...
                            <input class="form-control" type="hidden" value="'.$testimonials['testimonials_id'].'"  name="testimonials_id" required>
                            <input class="form-control" type="text" value="'.$testimonials['testimonials_name'].'" name="testimonials_name" required>
                        </div>
                        <div class="col form-group">
                            <label class="form-label" for="card-name">Designation <span class="text-danger">*</span></label>
                            <input class="form-control" type="text" value="'.$testimonials['testimonials_title'].'" name="testimonials_title" required>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col form-group">
                            <label class="form-label">Description <span class="text-danger">*</span></label>
                            <textarea name="testimonials_description" rows="7" class="form-control" required>'.$testimonials['testimonials_description'].'</textarea>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col form-group">
                            <label class="form-label">Image </label>
                            <input type="file" name="testimonials_image" accept="image/*" class="form-control"/>
                        </div>
                        <div class="col form-group">
                            ...
                            <select class="form-control select2-show-search form-select" required name="testimonials_status" data-placeholder="Select">
                                ...
                                foreach($statuses as $key => $status):
                                    echo '
                                    <option value="'.$key.'" '.($key == $testimonials['testimonials_status'] ? 'selected' : '').'>'.$status.'</option>';
                                endforeach;
                                echo '
                            </select>
                        </div>
